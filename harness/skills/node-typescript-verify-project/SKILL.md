---
name: node-typescript-verify-project
...
本 Step 不自行声称命令通过。绑定的 `node-typescript-project-quality-gate` 使用 `project-check` 自动识别 npm、Yarn 或 pnpm，并依次执行 Typecheck、Lint、Test、Build 和 `git diff --check`。
