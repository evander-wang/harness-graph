...
新增 `node-typescript-project-configuration` Workflow，使本地 Agent 能对 `projectRoot` 指向的本地目录完成两类任务：
...
1. `node-typescript-analyze-project`：检查项目现状并生成目标 Profile 和修改计划。
2. `node-typescript-configure-project`：根据已通过的计划创建或修改工程配置。
3. `node-typescript-verify-project`：准备确定性验证，不自行伪造命令结果。
4. `node-typescript-review-project-configuration`：审查一致性、安全性、范围和维护成本。
5. `node-typescript-deliver-project-configuration`：生成符合 Schema 的最终结果。
...
新项目的工具版本、目录、scripts、TypeScript、ESLint 和 CI 默认值固定在 `harness/skills/node-typescript-configure-project/BASELINE.md`。Workflow Version 不变时不应随 Agent 或执行日期漂移；偏离基线必须来自用户约束或项目类型，并记录原因。
...
- `node-typescript-project-plan-ready`：Profile、现状证据、修改范围、保留决定和验证方案完整时通过。
- `node-typescript-project-quality-gate`：执行 `project-check` 与 `git diff --check`。
- `node-typescript-project-review-result`：实际 Diff 与 Profile 一致、没有静默迁移和 Secret、文档与命令一致时通过。

配置或代码缺陷返回 `needs_changes`；缺少工具、权限或必须由用户决定的冲突返回 `blocked`。

## 路由边界

`node-typescript-project-configuration` 用于初始化、工程配置补齐、Node.js 版本统一、Lockfile 冲突、scripts、TypeScript、Lint、测试、构建、README、Docker 或 CI 一致性问题。
