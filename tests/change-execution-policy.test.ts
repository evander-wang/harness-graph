...
describe("change execution policy assets", () => {
  ...
  test("通用 Check 验证计划落盘并在 Review 对照接口文档", async () => {
    const planCheck = await readProjectFile("harness/checks/change-plan-ready/CHECK.md");
    const reviewCheck = await readProjectFile("harness/checks/change-review-result/CHECK.md");
  ...
  test("共享策略行为升级为可固定的 Workflow 版本", async () => {
    const result = await compileWorkflow({
      rootDir,
      workflowPath: "harness/workflows/change-execution-policy/workflow.yaml",
    });
