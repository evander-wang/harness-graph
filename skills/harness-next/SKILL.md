---
name: harness-next
description: 进入并执行当前项目安装的 Harness Next Workflow。Use when the user invokes $harness-next or /harness-next, names a Workflow or Alias, asks to run or recover a Workflow, or requests a project code or configuration change.
---
...
2. 定位包含 `harness-next/bin/harness-next` 的项目根目录；找不到唯一入口时停止并报告。
3. 从项目根执行 `./harness-next/bin/harness-next route`。
4. 要求命令成功，并且 stdout 只包含一个 JSON 对象。
5. 要求 `kind` 是 `harness-next.router-directive`，只加载 `routerSkillPath` 指向的 Skill。
...
- 不使用全局 Harness Next 或业务项目中的同名命令。
